...
export const validateTripSearchForm = (data: TripSearchFormData): TripSearchErrorCode | null => {
  if (!data.originStopId) {
    return TRIP_SEARCH_ERRORS.ORIGIN_REQUIRED;
  }
  if (!data.destinationStopId) {
    return TRIP_SEARCH_ERRORS.DESTINATION_REQUIRED;
  }
  if (data.originStopId === data.destinationStopId) {
    return TRIP_SEARCH_ERRORS.SAME_ORIGIN_DESTINATION;
  }
  if (!data.date.trim()) {
    return TRIP_SEARCH_ERRORS.DATE_REQUIRED;
  }
  if (!dateRegex.test(data.date.trim())) {
    return TRIP_SEARCH_ERRORS.DATE_INVALID_FORMAT;
  }
  const selectedDate = new Date(data.date);
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  if (selectedDate < today) {
    return TRIP_SEARCH_ERRORS.DATE_IN_PAST;
  }
  if (data.passengers < 1) {
    return TRIP_SEARCH_ERRORS.PASSENGERS_REQUIRED;
  }
  return null;
};
...
export const getErrorMessage = (errorCode: string): string => {
  ...
  if (errorCode in messageMap) {
    return messageMap[errorCode as TripSearchErrorCode];
  }
